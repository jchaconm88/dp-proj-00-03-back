...
export const MenusImportPanel: React.FC = () => {
        ...
        void (async () => {
          try {
            ...
            if (Array.isArray(parsed)) {
              ...
              menus = (parsed as { menus: unknown[] }).menus
            } else {
              setStatus('Formato inválido: array de menús o { "menus": [...] }.')
              return
            }

            const res = await fetch('/api/admin/import-menus', {
              method: 'POST',
              credentials: 'include',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ tenantId, menus }),
            })
            ...
            let msg = `${json.created ?? 0} creados, ${json.updated ?? 0} actualizados (upsert por ubicación).`
            if ((json.errors?.length ?? 0) > 0) {
              msg += ` ${json.errors!.length} con error.`
            }
            setStatus(msg)
          } catch {
            setStatus('JSON inválido. Usa menus.example.json del paquete de plantilla.')
          } finally {
            setBusy(false)
          }
        })()
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
        <strong style={{ fontSize: '14px' }}>Importar menús JSON</strong>
        <input
          ref={fileRef}
          type="file"
        ...
        <span style={{ fontSize: '13px', color: 'var(--theme-elevation-500)' }}>
          Ej. <code>menus.example.json</code> — upsert por <code>location</code> (header, footer…).
        </span>
      </div>
      {status ? (
        <p
